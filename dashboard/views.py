...
class DashboardView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for project in (
            ...
            pass_rate = None if total_tests == 0 else round((total_passed * 100 / total_tests), 2)
            pass_rate_display = "N/A" if pass_rate is None else f"{pass_rate:.2f}"
            ...
            project_rows.append(
                {
                    ...
                    "pass_rate_value": pass_rate if pass_rate is not None else -1,
        ...
        for regression in Regression.objects.select_related("project", "owner").annotate(
            ...
            pass_rate = None if total_tests == 0 else round(((regression.total_passed or 0) * 100 / total_tests), 2)
            fail_rate = None if total_tests == 0 else round(((regression.total_failed or 0) * 100 / total_tests), 2)
            regression_rows.append(
                {
                    ...
                    "pass_rate_display": "N/A" if pass_rate is None else f"{pass_rate:.2f}",
                    "fail_rate": fail_rate,
                    "fail_rate_display": "N/A" if fail_rate is None else f"{fail_rate:.2f}",
                    "pass_rate_value": pass_rate if pass_rate is not None else -1,
                    "fail_rate_value": fail_rate if fail_rate is not None else -1,
