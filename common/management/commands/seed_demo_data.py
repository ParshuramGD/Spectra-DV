...
class Command(BaseCommand):
    help = "Seed demo data for Spectra-DV"
    ...
    def handle(self, *args, **options):
        ...
        user1, _ = User.objects.get_or_create(
            username="engineer1", defaults={"email": "eng1@spectra_dv.local", "full_name": "Engineer One"}
        )
        if _:
            user1.set_password("password")
            user1.save()
        user2, _ = User.objects.get_or_create(
            username="engineer2", defaults={"email": "eng2@spectra_dv.local", "full_name": "Engineer Two"}
        )
        if _:
