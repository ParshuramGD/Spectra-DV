...
def main():
    ...
    groups = {}
    for res in results:
        key = (res["project_name"], res["regression_name"])
        if key not in groups:
            groups[key] = []
        groups[key].append(res)

    for (proj_name, reg_name), group_results in groups.items():
        payload = {
            "project_name": proj_name,
            "regression_name": reg_name,
            "run_name": f"{proj_name} Local Run",
            "branch_name": "main",
        ...
        print(f"Transmitting JSON payload securely to Spectra-DV for {proj_name} / {reg_name}...")
    ...
    total_tx = time.time() - start_parse
    print(f"Total end-to-end time: {total_tx:.3f} seconds.")
